...
##### reference files #####
build <- getOption("genome_build")
if(is.null(build)) build <- "hg38"
refdir <- getOption("refdir")
if(is.null(refdir)) refdir <- "/home/bl177/rnatea/ref"
...
# gene_file <- paste0("/home/bl177/rnatea/ref/annotation_data.", build, ".gtf")
gene_file <- file.path(refdir, "annotation_data.gtf")
# rmsk_gtf_file <- paste0("/n/data1/bch/genetics/lee/boram/ref/RepeatMasker/", build, "_rmsk.gtf")
rmsk_gtf_file <- file.path(refdir, "rmsk.gtf")
refTEfa <- getOption("refTEfa")
if(is.null(refTEfa)) refTEfa <- "/home/bl177/lee/boram/rnatea/ref/consensus_L1_ALU_SVA_HERV.fa"
...
filterSimpleRepeat.ctea <- function(ctea, Alength = 10, percentA = 0.8) {
  ...
  ctea[ori == "f",
       isPolyA := grepl(paste0("A{", Alength, "}"), seq) |
         str_count(seq, "A") / nchar(seq) > percentA
       ]
  ctea[ori == "r",
       isPolyA := grepl(paste0("T{", Alength, "}"), seq) |
         str_count(seq, "T") / nchar(seq) > percentA
       ]
  ctea <- ctea[isPolyA | !grepl("^[(ACGT)]+n$", family), ]
  ctea[isPolyA | !grepl("^([CT]+|[GA]+)$", seq), ]
}
...
polyTElocation.ctea <- function(ctea, clip_width = 5, build = getOption("genome_build")) {
  require(GenomicRanges)
  tegr <- if(isNamespaceLoaded("rtea")) {
    switch(build, hg19 = polyTE.hg19, hg38 = polyTE.hg38, stop("unknown build: ", build))
  } else {
    readRDS(polyTE_file)
  }
  tegr$class <- NULL
...
getClippedReads <- function(bamfile, chr, pos, ori = c("f", "r"),
  ...
  if(ori == "f") {
    ...
    sam <- sam[!duplicated(mcols(sam)$qname) & grepl("^[0-9]+S", cigar(sam))]
    ...
    sam <- sam[order(secondary, split, othersplit)]
    sam <- sam[!duplicated(mcols(sam)$qname) & grepl("S$", cigar(sam))]
...
countClippedReads.ctea <- function(ctea,
  ...
  lcnt <- mcmapply(
    ...
    FUN = function(i, bamfile, chr, pos, ori, seq) {
      ...
      possibleDup <- duplicated(paste(meta$sseq, meta$mpos))
      list(
...
geneticLocation <- function(chr, start, end = start, build = getOption("genome_build")) {
  annodata <- if(isNamespaceLoaded("rtea")) {
    switch(build, hg19 = anndata.hg19, hg38 = annodata.hg38, stop("unknown build: ", build))
  } else {
    writeLines(paste("Importing", gene_file))
    rtracklayer::import(gene_file, "gtf")
  }
...
annotate.ctea <- function(ctea,
  ...
  txannodata <- subset(annodata, type != "gene")
  cgr <- ctea[, GRanges(chr, IRanges(pos, width=1), c(r = "+", f = "-")[ori])]
  overlaptx <- findOverlaps(cgr, txannodata, ignore.strand = T)
  selectAnnodata <- function(overlap, data) {
    matchdata <- data.table(idx = queryHits(overlap),
...
localHardClip <- function(rtea,
  ...
  if(any(hclipped)){
    gr <- GRanges(rtea$chr[hclipped], IRanges(st[hclipped], en[hclipped]), "*")
...
TEcoordinate <- function(rtea, threads = getDTthreads()) {
  require(Biostrings)
  require(parallel)
  fa <- if(isNamespaceLoaded("rtea")) {
    consensusTE
  } else {
    writeLines("Reading TE fasta file...")
    readDNAStringSet(refTEfa)
  }
  names(fa) %<>% sub("\\s.*", "", .)
  TEclass <- rtea[, family]
